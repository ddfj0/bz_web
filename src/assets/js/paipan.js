...
const paipan = function(gd, yy, mm, dd, hh, mt=0, ss=0) {
    /**
     ...
     * @var array
     */
      var ctg = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸']; //char of TianGan
      /**
       * 五行
       */
      var cwx = ['金', '木', '水', '火', '土']; //char of WuXing
      /**
       * 天干对应五行
       * @var array
       */
      var tgwx = [1, 1, 3, 3, 4, 4, 0, 0, 2, 2]; 
      /**
       * 十二地支
       * @var array
       */
      var cdz = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥']; //char of DiZhi
      /**
       * 地支对应五行
       * @var array
       */
      var dzwx = [2, 4, 1, 1, 4, 3, 3, 4, 0, 0, 4, 2];
      /**
       * 地支藏干
       * @var array
       */
      var dzcg = [[9], [5,9,7], [0,2,4], [1], [4,1,9], [2,4,6], [3,5], [5,3,1], [6,8,4], [7], [4,7,3], [8,0]];
      /**
       * 十二生肖
       * @var array
       */
      var csa = ['鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗', '猪']; //char of symbolic animals
      /**
       * 十二星座
       * @var array
       */
      var cxz = ['水瓶座', '双鱼座', '白羊座', '金牛座', '双子座', '巨蟹座', '狮子座', '处女座', '天秤座', '天蝎座', '射手座', '摩羯座']; //char of XingZuo
      /**
       * 星期
       * @var array
       */
      var wkd = ['日', '一', '二', '三', '四', '五', '六']; //week day
      /**
       * 廿四节气(从春分开始)
       * @var array
       */
      var jq = ['春分', '清明', '谷雨', '立夏', '小满', '芒种', '夏至', '小暑', '大暑', '立秋', '处暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪', '大雪', '冬至', '小寒', '大寒', '立春', '雨水', '惊蛰']; //JieQi
      /**
       ...
       * @return boolean
       */
      var ValidDate = function(yy, mm, dd) {
       ...
       * @return number
       */
      var GetSolarDays = function(yy, mm){
          if (yy < -1000 || yy > 3000) { //适用于西元-1000年至西元3000年,超出此范围误差较大
              return 0;
          }
       ...
       * @return number
       */
      var GetLunarDays = function(yy, mm, isLeap){
          if (yy < -1000 || yy > 3000) { //适用于西元-1000年至西元3000年,超出此范围误差较大
              return 0;
          }
       ...
       */
      var Lunar2Solar = function(yy, mm, dd, isLeap) {
       ...
       */
      var Solar2Lunar = function(yy, mm, dd, hh) {
       ...
       */
      var GetGanZhi = function(yy, mm, dd, hh, mt=0, ss=0){
       ...
       * @return array
       */
      var GetInfo = function(gd, yy, mm, dd, hh, mt=0, ss=0){
          ...
          var gz = GetGanZhi(yy, mm, dd, hh, mt, ss);
          var tg = gz[0];
          var dz = gz[1];
          var jd = gz[2];
          var jq = gz[3];
          ...
          for(var i = 0; i < 12; i++){
              ret['big'] += ctg[big_tg[i]];
              ret['big'] += cdz[big_dz[i]];
              ret['big_start_time'].push(Julian2Solar(start_jdtime + i*10*365.25));
          }
          

          //// 这个120年的流年计算，坑太多了。自己再写一个吧。
          /*
          var iii;
          for(var i=1,j=0; ;i++){
              ...
              iii = i;
              //// 在一月和立春前，i要增加1
              if( mm == 1 || dz[1] == 1 ){  //立春前，阳历1月或者农历腊月，地支为丑
                iii = i + 1;
              }
          ...
          */
         var lngz;
         var syear = ret['start_time'][0];
          for( var ii = 0 ; ii < 100 ; ii++ ){
            lngz = GetGanZhi(syear + ii, 6, 1, 1, mt, ss);
          ...
          //console.log(ret['years']);
  
        ret['lunar'] = Solar2Lunar(yy, mm, dd, hh); //农历
          
          return ret;
      }
  
    return GetInfo(gd, yy, mm, dd, hh, mt, ss);
}
  
const fnComputeShishen = (daytg, gz) => {
    var rtn = '';
  
    var tg = '甲乙丙丁戊己庚辛壬癸'; //char of TianGan
      var dz = '子丑寅卯辰巳午未申酉戌亥'; //char of DiZhi
  
    var dayIdx = tg.indexOf(daytg);
    if( dayIdx == -1 ) return rtn;
  
    var gzIdx = tg.indexOf(gz);
    if( gzIdx >= 0 )   { //// 天干十神
...
const fnConvertLunarMonth = (month) => {
  var rtn;
  var numless = ['','一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
  rtn = numless[month];
  switch (month) {
    case 1:
      rtn = '正';
...
const fnConvertLunarDay = (day) => {
  var rtn = '';
  var numless = ['十','一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
  //// 数字农历日转换成中文 
  if( day <= 10 ) {
    rtn = '初' + numless[day];
  } else if( day < 20 ) {
    rtn = '十' + numless[day - 10];
  } else if( day == 20 ) {
    rtn = '二十';
  } else if( day < 30 ) {
    rtn = '廿' + numless[day - 20];
  } else {
    rtn = '三十';
  }
...
const fnMakeBazi = (birthday, sex) => {

  let bsex = sex == '女' ? true : false;
  let yy, mm, dd, hh;

  let bb = birthday.split(' ');
  bb[0].split('-').forEach((item, index) => {
      if( index == 0 ) yy = parseInt(item);
      if( index == 1 ) mm = parseInt(item);
      if( index == 2 ) dd = parseInt(item);
  });
  bb[1].split(':').forEach((item, index) => {
      if( index == 0 ) hh = parseInt(item);
  });

  let bzPaipan = paipan(bsex, yy, mm, dd, hh, 20, 30);

  var bazi = {};
  bazi.sex = sex;
  bazi.birthday = yy + '-' + mm + '-' + dd + ' ' + hh + ':20';

  /// 八字
  bazi.yearGan = bzPaipan.bazi.slice(0, 1);
  bazi.yearZhi = bzPaipan.bazi.slice(1, 2);
  bazi.monthGan = bzPaipan.bazi.slice(2, 3);
  bazi.monthZhi = bzPaipan.bazi.slice(3, 4);
  bazi.dayGan = bzPaipan.bazi.slice(4, 5);
  bazi.dayZhi = bzPaipan.bazi.slice(5, 6);
  bazi.hourGan = bzPaipan.bazi.slice(6, 7);
  bazi.hourZhi = bzPaipan.bazi.slice(7, 8);

  /// 农历
  bazi.lunarDate = bazi.yearGan + bazi.yearZhi + ' ' + bzPaipan.sx + '年 ' + fnConvertLunarMonth(bzPaipan.lunar[1]) + '月 ' + 
                      fnConvertLunarDay(bzPaipan.lunar[2])  + ' ' + bazi.hourZhi + '时';;

  /// 十神
  bazi.shenYearGan = fnComputeShishen( bazi.dayGan, bazi.yearGan );
  bazi.shenYearZhi = fnComputeShishen( bazi.dayGan, bazi.yearZhi );
  bazi.shenMonthGan = fnComputeShishen( bazi.dayGan, bazi.monthGan );
  bazi.shenMonthZhi = fnComputeShishen( bazi.dayGan, bazi.monthZhi );
  bazi.shenDayGan = '日';
  bazi.shenDayZhi = fnComputeShishen( bazi.dayGan, bazi.dayZhi );
  bazi.shenHourGan = fnComputeShishen( bazi.dayGan, bazi.hourGan );
  bazi.shenHourZhi = fnComputeShishen( bazi.dayGan, bazi.hourZhi );

  /// 地支藏干
  bazi.dzcgYearList  = fnComputeDzcg(bazi.dayGan, bazi.yearZhi);
  bazi.dzcgMonthList = fnComputeDzcg(bazi.dayGan, bazi.monthZhi);
  bazi.dzcgDayList   = fnComputeDzcg(bazi.dayGan, bazi.dayZhi);
  bazi.dzcgHourList  = fnComputeDzcg(bazi.dayGan, bazi.hourZhi);

  /// 大运
  bazi.bigyunList = [];
  let cc = bzPaipan.years.split('\n');
  for( var ii = 0 ; ii < 8 ; ii++ ) {
      let year ,age, tg, dz;
      year = bzPaipan.big_start_time[ii][0];
      age = year - yy;
      tg = bzPaipan.big.slice(ii * 2, ii * 2 + 1 );
      dz = bzPaipan.big.slice(ii * 2 + 1, ii * 2 + 2 );

      bazi.bigyunList.push({
          year: year,
          age : age,
          tg: tg,
          dz: dz,
          dzcg: fnComputeDzcg(bazi.dayGan, dz),
          liunian: cc[ii],
      });
  }
  return bazi;
...
export {paipan, fnComputeShishen, fnComputeDzcg, fnConvertLunarMonth, fnConvertLunarDay, fnMakeBazi};