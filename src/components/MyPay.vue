  ...
  <div class="main_area" :style="{ 'width': menuBar.isMobile ? 'calc(100vw - 6px)': '360px'}" >
    <div class="ma-2" style="width: 100%">
        <div class="d-flex " >
            ...
            <div >
                <span class="mx-2" style="color: white;"> 我的 </span>
            </div>
        </div>
        <div class="mt-2">  

            <v-row class="pa-0 ma-0" v-if="myOpenid == openid">
                <v-col cols="12" class="pa-0 ma-0">
                    <v-card :style="{ 'border': '1px solid #f1f4f6', 'width': menuBar.isMobile ? 'calc(100vw - 24px)': '340px' }">
                        <div style="height: 48px;">
                            <v-row>
                                <v-col cols="10" class="pa-0 ma-0">
                                    <div class="mt-4 ml-4">
                                        <v-icon @click="showUserList = !showUserList;">mdi-list-box</v-icon>
                                        用户记录
                                    </div>
                                </v-col>
                                <v-col cols="2" class="pa-0 ma-0">
                                    <div class="mt-4 d-flex justify-center">
                                        <v-icon @click="fnGetUserList();">mdi-refresh</v-icon>
                                    </div> 
                                </v-col>
                            </v-row>
                        </div>
                        <div v-if="showUserList" class="mx-2 mt-4" style="width: 100%; color: white;">
                            <el-table
                                :data="userList"
            ...
            <v-row class="pa-0 ma-0">
                <v-col cols="12" class="pa-0 ma-0">
                    <v-card :style="{ 'border': '1px solid #f1f4f6', 'height': '250px', 'width': menuBar.isMobile ? 'calc(100vw - 24px)': '340px' }">
                        <div >
                            <v-row>
                                <v-col cols="10" class="pa-0 ma-0">
                                    <div class="mt-4 ml-4">
            ...
            <v-card class="mt-2" :style="{ 'border': '1px solid #f1f4f6', 'width': menuBar.isMobile ? 'calc(100vw - 24px)': '340px' }" >
                <div >
                            <v-row>
                                ...
                                <v-col cols="2" class="pa-0 ma-0">
                                    <div class="mt-4 d-flex justify-center">
                                        <v-icon @click="fnPayListCopy();">mdi-content-copy</v-icon>
                                        <v-icon class="ml-1" @click="fnPayListDel();">mdi-delete</v-icon>
                                    </div> 
                                </v-col>
                            </v-row>
                        </div>
              ...
              <div v-if="payList.length > 0">
                <v-row class="pa-0 ma-0">
                    <v-col cols="12" class="pa-0 ma-0">
                            <div class="mt-2 mx-2">
                              <div class="d-flex justify-center">
                                <img :src="curUrl + '/wei.png?' + Math.floor(Math.random() * 100 + 1)" height=300 />
                              </div>
                            </div>
                    </v-col>
                </v-row>
...
onMounted(() => {
    if( openid.value != '' ) {
        ...
        ElMessage('微信id为空! 请重新登录，https://www.ddfj0.com/bz/');
    }
});
...
//const openid = ref('oYr3t6Rjtsa80OFin7Kb92xioQjE');

const myOpenid = ref('oYr3t6Rjtsa80OFin7Kb92xioQjE');
...
const fnGetUserList = () => {
    ...
    uuu.openid = openid.value;

    axios.post(curUrl.value + "/api/wxmp/user/list", uuu
    ).then((res) => {
        const oo = res.data;
        if( oo.code == 1000) {
            userList.value = JSON.parse(oo.result);
        }
        else {
            ElMessage(oo.msg);
        }
...
const fnUserChange = (row) => {
    if( row == null ) return;
    curUserList = row;
    username.value = curUserList.user_name;
    usersex.value = curUserList.sex;
    userlevel.value = curUserList.level;
    birthday.value = curUserList.birthday;
    regtime.value = curUserList.reg_time;
    paytime.value = curUserList.pay_time;
}
...
const fnUserUpdate = () => {
    ...
    uuu.appid = appid.value;
    uuu.openid = openid.value;
    uuu.user = curUserList;
    uuu.user.sex = usersex.value;
    uuu.user.level = userlevel.value;
    uuu.user.birthday = birthday.value;
    uuu.user.pay_time = paytime.value;

    axios.post(curUrl.value + "/api/wxmp/user/edit",
        uuu
    ).then((res) => {
        const oo = res.data;
        if( oo.code == 1000) {
            ElMessage('更新成功 - ' + oo.msg);
            fnGetUserList();
        }
        else {
            ElMessage(oo.msg);
...
const fnGetPayList = () => {
    ...
    axios.post(curUrl.value + "/api/user/wxmp/paylist", uuu
    ).then((res) => {
        const oo = res.data;
        if( oo.code == 1000) {
...
const fnPayChange = (row) => {
    ...
    textZiPing.value = JSON.parse(curPayList.fate_self);
}
...
const fnPayListDel = () => {
    ...
    ElMessageBox({
        ...
        message: '是否删除?', //MessageBox 消息正文内容
        ...
        uuu.appid = appid.value;
        uuu.openid = openid.value;
        uuu.user_pay_id = curPayList.user_pay_id;
        uuu.user_id = curPayList.user_id;

        axios.post(curUrl.value + "/api/user/wxmp/paydel",
            uuu
        ).then((res) => {
            const oo = res.data;
            if( oo.code == 1000) {
